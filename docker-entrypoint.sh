...
if [ -n "$PARAMETER_STORE_PREFIX" ]; then
    ...
    export AWS_ACCESS_KEY_ID=$(aws ssm get-parameters --name /qiiatan/stg/aws_access_key_id --with-decryption --query "Parameters[0].Value" --region ap-northeast-1 --output text)
    export AWS_SECRET_KEY_ID=$(aws ssm get-parameters --name /qiiatan/stg/aws_secret_access_key --with-decryption --query "Parameters[0].Value" --region ap-northeast-1 --output text)
    export AWS_SECRET_ACCESS_KEY=$(aws ssm get-parameters --name /qiiatan/stg/aws_secret_access_key --with-decryption --query "Parameters[0].Value" --region ap-northeast-1 --output text)
fi
